...
def init_db() -> None:
    ...
    try:
        ...
        """)
        # Co-occurrence table (from leo.py - semantic gravity)
        conn.execute("""
        ...
        """)
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_co_occurrence_word
            ON co_occurrence(word_id, count DESC)
        """)
        # Theme snapshots table (for flow tracking)
        conn.execute("""
        ...
        """)
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_theme_snapshots_time
            ON theme_snapshots(timestamp)
        """)
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_theme_snapshots_theme
...
# ═══════════════════════════════════════════════════════════════
# CO-OCCURRENCE — semantic gravity (from leo.py)
# ═══════════════════════════════════════════════════════════════

def ingest_co_occurrence(conn: sqlite3.Connection, tokens: List[str], window_size: int = 5) -> None:
    """
    Build co-occurrence matrix from token sequence (sliding window).
    
    For each word, remember which words appeared nearby historically.
    This creates semantic gravity: words that resonate together, stay together.
    """
    if not tokens:
        return
    ...
    for i, center_word in enumerate(tokens):
        ...
        for j in range(start, end):
            ...
            conn.execute(
                ...
                """,
                (center_word.lower(), context_word.lower())
            )
...
def load_co_occurrence(conn: sqlite3.Connection) -> Dict[str, Dict[str, int]]:
    ...
    Returns: word -> {context_word -> count}
    """
    co_occur: Dict[str, Dict[str, int]] = {}
    rows = conn.execute("SELECT word_id, context_id, count FROM co_occurrence").fetchall()
    
    for word_id, context_id, count in rows:
        row_map = co_occur.setdefault(word_id, {})
        row_map[context_id] = row_map.get(context_id, 0) + count
    
    return co_occur


# ═══════════════════════════════════════════════════════════════
# THEME CLUSTERING — semantic islands (from leo.py ThemeLayer)
# ═══════════════════════════════════════════════════════════════
...
def cluster_themes(
    ...
    """
    Agglomerative clustering of words into semantic themes.
    
    Uses Jaccard similarity over co-occurrence contexts.
    Words that share similar context neighbors cluster together.
    
    Returns: List of theme sets (each set = words in that theme)
    """
    if not co_occur:
        return []
    
    # Build context sets for each word
    word_contexts: Dict[str, Set[str]] = {}
    for word, contexts in co_occur.items():
        # Top-K contexts by count (to avoid noise)
        sorted_contexts = sorted(contexts.items(), key=lambda x: x[1], reverse=True)
        top_contexts = {ctx for ctx, _ in sorted_contexts[:20]}  # Top 20 contexts
        if top_contexts:
            word_contexts[word] = top_contexts
    
    if not word_contexts:
        return []
    
    # Agglomerative clustering: start with each word as its own cluster
    themes: List[Set[str]] = [{word} for word in word_contexts.keys()]
    ...
    while changed:
        ...
        for i in range(len(themes)):
            for j in range(i + 1, len(themes)):
                ...
                # Compute average similarity between themes
                similarities = []
                for word_i in theme_i:
                    for word_j in theme_j:
                        ctx_i = word_contexts.get(word_i, set())
                        ctx_j = word_contexts.get(word_j, set())
                        sim = jaccard_similarity(ctx_i, ctx_j)
        ...
        if best_merge:
            i, j = best_merge
            themes[i] = themes[i] | themes[j]
            themes.pop(j)
            changed = True
    
    # Filter small themes
    themes = [t for t in themes if len(t) >= min_theme_size]
...
def reassemble_corpse_bootstrap(leaves: List[str]) -> str:
    """
    Enhanced reassembly with CO-OCCURRENCE blending (from leo.py):
    - SEED_BIGRAMS (structural patterns from corpus)
    - corpse_bigrams (learned successful chains)
    - co_occurrence (semantic gravity: which words resonate together)
    - phonetic chaos (unpredictability)
    
    When multiple strong grammatical candidates exist, blend:
    - 70% grammar (bigram weight)
    ...
    # Generate corpse using weighted random selection with CO-OCCURRENCE blending
    current = random.choice(leaves)
    result = [current]
    seen = {current.lower()}
    ...
    for _ in range(target_len):
        ...
        if valid_options:
            # CO-OCCURRENCE BLENDING: If multiple strong candidates, use semantic gravity
            if len(valid_options) > 1 and current.lower() in co_occur:
                ...
                if len(strong_indices) > 1:
                    ...
                    blended_weights = []
                    for i, (word, gram_weight) in enumerate(valid_options):
                        sem_bonus = float(co_occur[current.lower()].get(word.lower(), 0))
                    ...
                    # Single strong candidate or no co-occurrence data - use grammar only
                    words, weights = zip(*valid_options)
    ...
    # Ingest co-occurrence from generated corpse (feed back into field)
    conn = sqlite3.connect(DB_PATH)
    try:
        ingest_co_occurrence(conn, result)
    finally:
        conn.close()
...
# ═══════════════════════════════════════════════════════════════
# PRESENCE PULSE — situational awareness for autopsy
# ═══════════════════════════════════════════════════════════════

class PresencePulse(NamedTuple):
    """
    Composite presence metric for autopsy process.
    
    - novelty: how unfamiliar the mutations are (0-1)
    - arousal: emotional charge from prompt/corpse (0-1)
    - entropy: uncertainty during mutation selection (0-1)
    - pulse: weighted composite (0-1)
    """
    novelty: float
    arousal: float
    entropy: float
...
def compute_mutation_novelty(
    prompt_tokens: List[str],
    mutation_trees: List[Node],
    known_mutations: Set[str]
) -> float:
    """
    Compute novelty: how many mutations are unknown to the morgue.
    
    High novelty = unfamiliar mutations (Sorokin exploring new territory)
    Low novelty = familiar mutations (Sorokin repeating patterns)
    """
    if not mutation_trees:
        return 0.5
    
    all_mutations: Set[str] = set()
    for tree in mutation_trees:
        leaves = collect_leaves(tree)
        all_mutations.update(w.lower() for w in leaves)
    
    if not all_mutations:
        return 0.5
    
    known_count = sum(1 for m in all_mutations if m in known_mutations)
    novelty = 1.0 - (known_count / len(all_mutations))
    
    return max(0.0, min(1.0, novelty))


def compute_prompt_arousal(tokens: List[str], emotion_map: Optional[Dict[str, float]] = None) -> float:
    """
    Compute arousal from emotional charge in prompt.
    
    Heuristics:
    - ALL-CAPS words (length >= 2)
    - Exclamation marks nearby
    - Token repetitions
    
    No sentiment models. Just structural intensity.
    """
    if not tokens:
        return 0.0
    ...
    for i, tok in enumerate(tokens):
        ...
        if "!" in local or any("!" in t for t in local):
            score += 1.0
...
def compute_mutation_entropy(
    mutation_trees: List[Node],
    all_candidates: List[str]
) -> float:
    """
    Compute entropy: uncertainty during mutation selection.
    
    Measures how "confused" Sorokin is when choosing mutations.
    High entropy = many equally likely paths (chaos)
    Low entropy = clear favorite mutations (deterministic)
    """
    if not mutation_trees:
        return 0.5
    
    # Collect all branch choices (word -> [children])
    branch_counts: List[float] = []
    
    def _collect_branch_counts(node: Node):
        if node.children:
            branch_counts.append(float(len(node.children)))
            for child in node.children:
                _collect_branch_counts(child)
    
    for tree in mutation_trees:
        _collect_branch_counts(tree)
    
    if not branch_counts:
        return 0.5
    
    # Compute entropy of branch distribution
    raw_entropy = distribution_entropy(branch_counts)
    max_entropy = math.log(len(branch_counts) + 1e-12) if branch_counts else 1.0
...
def compute_autopsy_resonance(
    tree_text: str,
    corpse: str,
    original_prompt: str,
    mutation_trees: Optional[List[Node]] = None,
    known_mutations: Optional[Set[str]] = None
) -> Dict[str, float]:
    """
    Compute resonance score with PRESENCE PULSE integration.
    ...
    Presence metrics (from leo.py):
    - Novelty: how unfamiliar the mutations are
    - Arousal: emotional charge from prompt/corpse
    - Entropy: uncertainty during mutation selection
    - Pulse: composite presence score
    
    Pure structural metrics. NO embeddings. NO semantics.
    """
    corpse_words = corpse.strip().split()
    prompt_tokens = tokenize(original_prompt)
    ...
    # 4. PRESENCE PULSE (from leo.py)
    if mutation_trees is not None and known_mutations is not None:
        novelty = compute_mutation_novelty(prompt_tokens, mutation_trees, known_mutations)
        arousal = compute_prompt_arousal(prompt_tokens)
        entropy = compute_mutation_entropy(mutation_trees, prompt_tokens)
        pulse_obj = compute_presence_pulse(novelty, arousal, entropy)
        pulse = pulse_obj.pulse
    else:
        # Fallback if trees not provided
        novelty = 0.5
        arousal = compute_prompt_arousal(prompt_tokens)
        entropy = 0.5
        pulse = 0.4
    
    # Overall resonance (weighted combination: 50% structural, 50% presence)
    structural_resonance = 0.4 * phonetic_diversity + 0.3 * semantic_coherence + 0.3 * syntactic_flow
    overall = 0.5 * structural_resonance + 0.5 * pulse
...
def render_autopsy_bootstrap(prompt: str, words: List[str], trees: List[Node], 
    ...
    # PRESENCE PULSE metrics (from leo.py)
    if 'novelty' in resonance:
        out.append("")
        out.append("PRESENCE PULSE:")
        out.append(f"  Novelty:  {_render_bar(resonance['novelty'])} {resonance['novelty']:.3f}")
        out.append(f"  Arousal:  {_render_bar(resonance['arousal'])} {resonance['arousal']:.3f}")
        out.append(f"  Entropy:  {_render_bar(resonance['entropy'])} {resonance['entropy']:.3f}")
        out.append(f"  Pulse:    {_render_bar(resonance['pulse'])} {resonance['pulse']:.3f}")
...
async def sorokin_autopsy_bootstrap(prompt: str) -> str:
    ...
    # Load known mutations for presence pulse
    conn = sqlite3.connect(DB_PATH)
    try:
        known_mutations_rows = conn.execute(
            "SELECT DISTINCT target_word FROM mutation_templates"
        ).fetchall()
        known_mutations = {row[0].lower() for row in known_mutations_rows}
        
        # THEME CLUSTERING: Group mutations into semantic islands
        co_occur = load_co_occurrence(conn)
        themes = cluster_themes(co_occur, threshold=0.3, min_theme_size=3)
        
        # FLOW TRACKING: Record theme activity during this autopsy
        try:
            from gowiththeflow import track_autopsy_themes
            track_autopsy_themes(conn, all_leaves, themes, co_occur)
        except ImportError:
            pass  # Optional module
    finally:
        conn.close()
    
    # Compute resonance with PRESENCE PULSE
    resonance = compute_autopsy_resonance(
        basic_report, corpse, short,
        mutation_trees=trees,
